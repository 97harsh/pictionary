    ...
    <div className="w-full space-y-2">
      {sortedTeams.map((team, index) => (
        <Card
          ...
          className={cn(
            'p-3 transition-all',
            team.id === currentTeamId ? 'border-accent ring-2 ring-accent' : 'bg-card/50',
        ...
        >
          <div className="flex justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <span
                className={cn("font-bold text-lg w-6 text-center", index === 0 && 'text-yellow-400')}
              >
                {index === 0 ? <Trophy className="h-5 w-5 mx-auto"/> : index + 1}
              </span>
